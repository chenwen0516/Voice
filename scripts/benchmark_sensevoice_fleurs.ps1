...
$manifest = Join-Path $root "samples\datasets\fleurs-zh\manifest.csv"
$outputDir = Join-Path $root "samples\datasets\fleurs-zh-sensevoice-small"
...
if (-not (Test-Path $manifest)) {
    throw "FLEURS manifest not found. Run .\scripts\download_small_dataset.ps1 --dataset fleurs-zh --limit 100 --max-mb 100 first."
}
